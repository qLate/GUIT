...
struct wl_surface* srfc2;
struct wl_buffer* bfr;
struct wl_shm* shm;
struct xdg_wm_base* sh;
struct xdg_toplevel* top;
struct xdg_toplevel* top2;
struct wl_seat* seat;
...
void draw()
{
    ...
    wl_surface_attach(srfc2, bfr, 0, 0);
    wl_surface_damage_buffer(srfc2, 0, 0, w, h);
    wl_surface_commit(srfc2);
}
...
struct xdg_toplevel_listener top_list2= {
        .configure = top_conf,
        .close = top_cls
};
...
int8_t main()
{
    ...
    srfc2 = wl_compositor_create_surface(comp);
    struct wl_callback* cb2 = wl_surface_frame(srfc);
    wl_callback_add_listener(cb2, &cb_list, 0);
    ...
    struct xdg_surface* xrfc2 = xdg_wm_base_get_xdg_surface(sh, srfc2);
    xdg_surface_add_listener(xrfc2, &xrfc_list, 0);
    top2 = xdg_surface_get_toplevel(xrfc2);
    xdg_toplevel_add_listener(top2, &top_list2, 0);
    xdg_toplevel_set_title(top2, "wayland client 2");
    wl_surface_commit(srfc2);
	...
	xdg_toplevel_destroy(top2);
	xdg_surface_destroy(xrfc);
	xdg_surface_destroy(xrfc2);
	wl_surface_destroy(srfc);
	wl_surface_destroy(srfc2);
